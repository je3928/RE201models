...
- create the presets
*/
PluginCore::PluginCore() : tapemodel(std::make_unique<RE201model4>())
{
...
bool PluginCore::reset(ResetInfo& resetInfo)
{
	...
	tapemodel->reset(resetInfo.sampleRate, OSamount[osAmount], solverorder);
...
bool PluginCore::processAudioFrame(ProcessFrameInfo& processFrameInfo)
{
	...
	tapemodel->updateParams(bass, treble, intensity, repeatRate, playheadstates, delayEnabled[delaySetting], echovolume, reverbEnabled[delaySetting], reverbvolume, InputLevel, wetDry);
       ...
       processFrameInfo.channelIOConfig.outputChannelFormat == kCFMono)
    {
		...
		processFrameInfo.audioOutputFrame[0] = tapemodel->processSample(processFrameInfo.audioInputFrame[0]);

		m_fMeterValue = processFrameInfo.audioOutputFrame[0];
       ...
       processFrameInfo.channelIOConfig.outputChannelFormat == kCFStereo)
    {
		...
		tapemodel->processFrame(processFrameInfo.audioInputFrame, processFrameInfo.audioOutputFrame, true);
       ...
       processFrameInfo.channelIOConfig.outputChannelFormat == kCFStereo)
    {
		...
		tapemodel->processFrame(processFrameInfo.audioInputFrame, processFrameInfo.audioOutputFrame, false);
...
bool PluginCore::initPluginParameters()
{
	...
	piParam = new PluginParameter(controlID::bass, "Bass", "", controlVariableType::kFloat, 0.001000, 0.999000, 0.130000, taper::kAntiLogTaper);
	...
	piParam = new PluginParameter(controlID::treble, "Treble", "", controlVariableType::kFloat, 0.001000, 0.999000, 0.130000, taper::kAntiLogTaper);
	piParam->setParameterSmoothing(true);
	piParam->setSmoothingTimeMsec(20.00);
	...
	// --- meter control: LED
	piParam = new PluginParameter(controlID::m_fMeterValue, "LED", 10.00, 200.00, ENVELOPE_DETECT_MODE_PEAK, meterCal::kLogMeter);
	piParam->setBoundVariable(&m_fMeterValue, boundVariableType::kFloat);
	addPluginParameter(piParam);

	// --- discrete control: Solver Order
	piParam = new PluginParameter(controlID::solverorder, "Solver Order", "Second,Fourth", "Fourth");
	piParam->setBoundVariable(&solverorder, boundVariableType::kInt);
	piParam->setIsDiscreteSwitch(true);
	addPluginParameter(piParam);

	// --- discrete control: OS Amount
	piParam = new PluginParameter(controlID::osAmount, "OS Amount", "Two,Four,Six,Eight", "Two");
	piParam->setBoundVariable(&osAmount, boundVariableType::kInt);
	piParam->setIsDiscreteSwitch(true);
	addPluginParameter(piParam);
	...
	// --- controlID::solverorder
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(805306368);
	setParamAuxAttribute(controlID::solverorder, auxAttribute);

	// --- controlID::osAmount
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(805306368);
	setParamAuxAttribute(controlID::osAmount, auxAttribute);
...
bool PluginCore::initPluginPresets()
{
	...
	setPresetParameter(preset->presetParameters, controlID::bass, 0.500000);
	setPresetParameter(preset->presetParameters, controlID::treble, 0.500000);
	...
	setPresetParameter(preset->presetParameters, controlID::solverorder, 1.000000);
	setPresetParameter(preset->presetParameters, controlID::osAmount, -0.000000);
